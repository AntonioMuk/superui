...
- `自动引导`：会在会话开始或上下文加载时引导 agent 读取 `superui`。
- `端到端验证`：已在对应运行时完成最小验收测试。
...
| 通用 agent / OpenAI Agents | 是 | 手动/框架适配 | 依赖 AGENTS.md | 未验证 | 读取 `AGENTS.md` 后进入 `skills/superui/SKILL.md`。 |
...
1. Agent 先加载 `skills/superui/SKILL.md`。
2. Agent 确定 `<ARTIFACT_ROOT>`。
3. Agent 读取用户偏好或只读模板。
4. Agent 路由到 `superui-design-md`。
5. Agent 不直接跳到页面代码实现。
