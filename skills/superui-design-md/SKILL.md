...
Step 2  案例参照 → 从 74 个品牌案例中找风格参考
Step 3  生成 DESIGN.md → 按模板填充
Step 4  生成预览 HTML → 双色预览
Step 5  Lint 校验 → @google/design.md CLI
```
...
按以下 11 个段落依次撰写，每段至少 3 个要点：

1. **Overview**：一句话核心特征 + 品牌调性 + 设计哲学
2. **Colors**：主色/功能色/中性色的使用场景和对比度声明
3. **Typography**：字体选择理由 + 层级视觉逻辑 + 字重分配
4. **Layout**：栅格 + 分区模式 + 间距系统的使用规则
5. **Elevation & Depth**：阴影使用场景 + z-index 分层
6. **Shapes**：圆角体系 + 图标风格
7. **Components**：按钮、输入框、卡片、导航、弹窗的详细规范（每个组件含变体×状态矩阵）
8. **Interaction Loop** ⭐：CRUD 完整闭环、加载态策略、错误处理、乐观更新
9. **Do's and Don'ts**：每个核心组件至少 1 对正反例
10. **Responsive Behavior**：断点行为、移动端特殊规则
11. **Agent Prompt Guidelines**：给 AI 的指令（禁止项 `MUST NOT` + 强制项 `SHALL`）
