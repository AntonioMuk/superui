...
4. **系统级长期偏好**：用户偏好不写入项目产物目录，优先使用 `~/.codex/superui/USER_PREFERENCES.md`；不可写时使用 `skills/superui-shared/USER_PREFERENCES.md` 作为本地回退。
...
1. 尝试读取 `~/.codex/superui/USER_PREFERENCES.md`。
2. 如果系统级文件不可用，读取 `skills/superui-shared/USER_PREFERENCES.md`。
3. 将稳定偏好作为软约束；与当前用户需求冲突时，以当前请求为准并记录冲突原因。
