...
5. 读取 `skills/superui-shared/TASK_MANAGEMENT.md`，创建或更新 `<ARTIFACT_ROOT>/todo.md`、`decision-log.md`、`delivery-checklist.md` 和 `pipeline-status.md`。
...
- 便携源文件：`<ARTIFACT_ROOT>/todo.md`、`decision-log.md`、`delivery-checklist.md`、`pipeline-status.md`。
- 原生清单同步：如果当前 agent 有 TodoWrite、plan、task list、memory 或 issue 工具，可把同一批任务镜像进去；便携源文件仍然是跨 agent 交接基准。
- 状态词统一：`pending`、`in_progress`、`blocked`、`done`；默认最多一个 `in_progress`。
- 决策留痕：记录可审计的决策、证据、假设、取舍和阻塞，不记录或输出私有逐字推理过程。
- 完成条件：没有更新 `delivery-checklist.md` 和必要验证证据时，不声明阶段或任务完成。
